...
|aria_config|N/A|Yes|This is the "parent folder" of any parameters that should get passed to aria2. Any [command line option](http://aria2.sourceforge.net/manual/en/html/aria2c.html#options) available in aria2 can be used by removing the two dashes (--) in front of the command name and changing ```key=value``` to ```key: value```. Any parameter can contain the name of any [entry field](http://flexget.com/wiki/Entry) using [jinja2 notation](http://flexget.com/wiki/Plugins/set#Jinja2Templating) and will be parsed prior to passing to aria2. See sample configuration below.|
